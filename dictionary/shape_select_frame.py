...
class ShapeSelectFrm(tk.Frame):
    ...
    def __init__(self, parent, imgdir, **options):
        ...
        self.var = tk.IntVar()  # tracks a shape selected in the popup window
        self.var.set(0)             # 0 means there's no shape selected
        self.var1 = tk.IntVar()  # tracks a shape displayed on the 1st place
        self.var1.set(0)
        self.var2 = tk.IntVar()  # tracks a shape displayed on the 2nd place
        self.var2.set(0)
    ...
    def makeLabel(self):
        ...
        tk.Label(self,
                 text=self.title,
                 bg=self.bgcolor).grid(column=1, row=0,
                                       sticky=tk.E+tk.W,
                                       pady=(0, self.labPady))
    ...
    def makeSelectionFrm(self):
        ...
        self.selectionfrm = tk.Frame(self,
                                     width=selfrmwidth,
                                     height=selfrmheight,
                                     bg=self.bgcolor,
                                     borderwidth=2,
                                     relief='groove')
        self.selectionfrm.grid(column=1, row=1, rowspan=2, sticky=tk.W)

        self.selectionfrm.columnconfigure(0, minsize=self.labwidth +
                                          2*self.labborder)
        self.selectionfrm.rowconfigure(0, minsize=self.labheight +
                                       2*self.labborder)
        self.selectionfrm.columnconfigure(1, minsize=self.sepwidth)
        self.selectionfrm.columnconfigure(2, minsize=self.labwidth +
                                          2*self.labborder)

        # Create a separator. On creation, it's not visible.
        # (It has the same color as selectionfrm background.)
        self.sep = tk.Frame(self.selectionfrm, bg=self.bgcolor)
        self.sep.grid(column=1, row=0, sticky=tk.N+tk.S)
    ...
    def showCaption(self, text):
        ...
        self.caption = tk.Toplevel()
        msg = tk.Message(self.caption,
                         text=text,
                         width=self.msgWidth,
                         bg=self.bgcolor)
        msg.grid()
    ...
    def createPopup(self):
        ...
        self.popupWin = tk.Toplevel(bg=self.popupBgColor)
    ...
    def makePopupWinButtons(self):
        ...
        buttonsfrm = tk.Frame(self.popupWin, bg=self.popupBgColor)
        buttonsfrm.grid(column=0, row=2, sticky=tk.E)
        ...
        submitButton.grid(column=0, row=0,
                          sticky=tk.E+tk.W,
                          padx=(0, 15), pady=10)
        submitButton['compound'] = tk.LEFT  # image to the left of button text
    ...
    def redrawPlace1(self):
        ...
        if self.var1.get() != 0:
            ...
            if self.sel1 is not None:
                self.sel1.destroy()
            self.sel1 = tk.Label(self.selectionfrm,
                                 image=image,
                                 borderwidth=0)
    ...
    def redrawPlace2(self):
        ...
        if self.var2.get() != 0:
            ...
            if self.sel2 is not None:
                self.sel2.destroy()
            self.sel2 = tk.Label(self.selectionfrm,
                                 image=image,
                                 borderwidth=0)
    ...
    def makeLabels(self):
        ...
        if self.labels == []:
            for i in range(len(self.shapes)):
                ...
                lab = tk.Label(self.scrollfrm.interior,
                               image=self.images[i],
                               borderwidth=0)
    ...
    def createSeparators(self):
        ...
        for i in range(self.numcols - 1):
            sep = tk.Frame(self.scrollfrm.interior, bg=self.sepcolor)
            sep.grid(column=2*i + 1,
                     row=0,
                     # num of rows:
                     rowspan=(len(self.shapes)//self.numcols)*2+1,
                     sticky=tk.N+tk.S)

        # create horizontal separators between the labels
        for i in range(len(self.shapes) // self.numcols):  # num of full rows
            sep = tk.Frame(self.scrollfrm.interior, bg=self.sepcolor)
            sep.grid(column=0,
                     row=2*i + 1,
...
class PassiveShapeSelectFrm(ShapeSelectFrm):
    ...
    def deactivate(self):
        ...
        self.darkscreen = tk.Label(self.selectionfrm,
                                   bg='grey90',
                                   borderwidth=0)
