...
class PlacementFrm(tk.Frame):
    ...
    def makeWidgets(self):
        ...
        self.hintIcon = tk.Label(self, image=self.icon, bg=self.bgcolor)
        ...
        self.image = tools.getImage(self.canvasImagePath,
                                    width=self.canvasWidth,
                                    height=self.canvasHeight)
        self.canvas.create_image(2, 2, image=self.image, anchor=tk.NW)
    ...
    def showHint(self, event):
        ...
        self.hint = tk.Toplevel()
        msg = tk.Message(self.hint,
                         text=self.hintText,
                         width=140,  # the actual width will be different
                         # because the words are not split at the end of line
                         bg=self.bgcolor)
        msg.grid()

        # set hint font if not defined yet
        if not self.captFont:
            self.setCaptFont(msg)
